...
function* _fetch(basedir, distdir) {
     ...
     * 读取配置信息
     */
    const { repo: { host, port }, dependencies } = require(path.join(basedir, 'config/config.navee'));
    ...
    for (const dependency of dependencies) {
        ...
        const { dependencies } = require(path.join(basedir, 'config/config.navee'));
...
function install() {
     ...
     * 仓库ip、端口
     */
    const { repo: { host, port } } = require(path.join(basedir, 'config/config.navee'));
    // 模块信息
